...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $assigned_location = mysqli_real_escape_string($conn, trim($_POST['location']));
    ...
    if (empty($name) || empty($email) || empty($phone_no) || empty($address) || 
        empty($occupation) || empty($dob) || empty($assigned_location) || 
        empty($medical_info) || empty($blood_group) || empty($allergy) || 
        empty($chronic_disease)) {
        $error_message = "All fields are required.";
    } else {
        // Check if email already exists
        $check_email = "SELECT id FROM Volunteer WHERE email = '$email'";
        ...
        if ($result->num_rows > 0) {
            ...
            if ($conn->query($sql) === TRUE) {
                $success_message = "Registration successful! Your volunteer ID is: " . $conn->insert_id;
...
<head>
  ...
  <title>Volunteer Registration | ResQ</title>
...
<!-- Include Navbar -->
  ...
  <section class="container" style="margin-top: 160px;">
    <div class="form-container">
      ...
      <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" onsubmit="return validateForm(event)">
        ...
        <input type="text" name="location" id="location" placeholder="Assigned Location (District, City)" class="form-input" 
               value="<?php echo isset($_POST['location']) ? htmlspecialchars($_POST['location']) : ''; ?>" required />
        <div id="locationError" class="field-error">Please enter your assigned location</div>
  ...
  <script>
    ...
    function validateForm(event) {
      ...
      if (!location) {
        showError('locationError', 'Please enter your assigned location');
        isValid = false;
      }
