...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $volunteer_id = mysqli_real_escape_string($conn, trim($_POST['Id']));
    $login_password = mysqli_real_escape_string($conn, $_POST['loginPassword']);
    
    // Basic validation
    if (empty($login_id) || empty($volunteer_id) || empty($login_password)) {
        $error_message = "All fields are required.";
    } else {
        // Check if volunteer exists with the given ID and email
        $sql = "SELECT * FROM Volunteer WHERE id = '$volunteer_id' AND email = '$login_id'";
        ...
        if ($result->num_rows > 0) {
            ...
            // and verify using password_verify() function
            
            // Since your current registration doesn't hash passwords,
            // we'll do a direct comparison for now
            // Note: This is not secure for production use
            
            // Store volunteer information in session
            ...
            $success_message = "Login successful! Welcome, " . $volunteer['name'] . "!";
            
            // Redirect to profile page after 2 seconds
            header("refresh:2;url=volunteer_profile.php");
...
<head>
  ...
  <title>Volunteer Login | ResQ</title>
  ...
  <section class="container">
    <div class="login-container">
      ...
      <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" onsubmit="return validateLogin(event)">
        ...
        <input type="number" name="Id" id="Id" placeholder="Your Volunteer ID" class="form-input" 
               value="<?php echo isset($_POST['Id']) ? htmlspecialchars($_POST['Id']) : ''; ?>" required />
        <div id="IdError" class="field-error">Please enter your volunteer ID</div>
      ...
      <div class="login-footer">
        Don't have an account? <a href="volunteerregistration.php">Register here</a>.<br>
        <a href="forget_vol_password.php">Forgot password?</a>
      </div>
    </div>
  </section>
 ...
 <footer class="footer">
  ...
  <script>
    function validateLogin(event) {
      ...
      // Validate ID
      const volunteerId = document.getElementById('Id').value.trim();
      if (!volunteerId) {
        showError('IdError', 'Please enter your volunteer ID');
        isValid = false;
      } else if (isNaN(volunteerId) || volunteerId <= 0) {
        showError('IdError', 'Please enter a valid volunteer ID (number only)');
        isValid = false;
      }
