...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['email'])) {
        ...
        if (empty($email)) {
            ...
            // Check if email exists in volunteer table
            $sql = "SELECT id, name FROM Volunteer WHERE email = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                $_SESSION['reset_volunteer_id'] = $volunteer['id'];
                $_SESSION['reset_volunteer_email'] = $email;
                $_SESSION['reset_volunteer_name'] = $volunteer['name'];
                header("Location: forget_vol_password.php?step=2");
                exit();
            } else {
                $error = "No volunteer account found with this email address.";
            }
        }
    } elseif (isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
        // Step 2: Password reset
        if (!isset($_SESSION['reset_volunteer_id'])) {
            header("Location: forget_vol_password.php?step=1");
            exit();
        }
        ...
        if (empty($errors)) {
            ...
            $volunteer_id = $_SESSION['reset_volunteer_id'];
            
            $sql = "UPDATE Volunteer SET password = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $hashed_password, $volunteer_id);
            
            if ($stmt->execute()) {
                // Clear session variables
                unset($_SESSION['reset_volunteer_id']);
                unset($_SESSION['reset_volunteer_email']);
                unset($_SESSION['reset_volunteer_name']);
                
                header("Location: forget_vol_password.php?step=3");
                exit();
            } else {
...
<head>
    ...
    <meta name="description" content="ResQ Volunteer Password Reset - Reset your volunteer account password">
    <title>Reset Volunteer Password | ResQ: Chaos To Control</title>
    ...
    <div class="container">
        <div class="reset-container">
            ...
            <?php if ($step == 1): ?>
                ...
                <h1 class="reset-title">Reset Volunteer Password</h1>
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="reset-form">
                    ...
                    <a href="volunteerlogin.php" class="btn btn-secondary">
            ...
            <?php elseif ($step == 2): ?>
                ...
                <p class="reset-description">
                    Account found for: <strong><?php echo htmlspecialchars($_SESSION['reset_volunteer_name']); ?></strong><br>
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>?step=2" class="reset-form">
                    ...
                    <a href="forget_vol_password.php?step=1" class="btn btn-secondary">
            ...
            <?php elseif ($step == 3): ?>
                ...
                <div class="success-content">
                    ...
                    <p class="success-message">
                        Your volunteer account password has been successfully updated. 
                        You can now log in with your new password.
                    </p>

                    <a href="volunteerlogin.php" class="btn btn-primary">
                        <i class="fa-solid fa-sign-in-alt"></i> Login to Your Account
                    </a>

                    <a href="resq.html" class="btn btn-secondary">
